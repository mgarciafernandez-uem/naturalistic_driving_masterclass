...
def federated_averaging(models, weights=None):
    ...
    for key in models[0].state_dict().keys():
        ...
        stacked_params = torch.stack([model.state_dict()[key] for model in models], dim=0)
        view_shape = [len(models)] + [1] * (stacked_params.dim() - 1)
        avg_state_dict[key] = (stacked_params * weights.view(*view_shape)).sum(dim=0)
    
    return avg_state_dict

def train_fedprox(models, train_loaders, num_epochs=2500, lr=1e-5, mu=1e-3):
    """Train models using FedProx (local proximal regularization + server averaging)."""
    criterion = nn.MSELoss()
    ...
    for epoch in range(num_epochs):
        global_state = {k: v.detach().clone() for k, v in models[0].state_dict().items()}

        # Train each model on its local data for one epoch
        epoch_losses = []
        
        for model, train_loader, optimizer in zip(models, train_loaders, optimizers):
            model.train()
            running_loss = 0.0
            ...
            for images, angles in train_loader:
                ...
                optimizer.zero_grad()
                outputs = model(images)
                base_loss = criterion(outputs, angles)

                prox_term = 0.0
                for name, param in model.named_parameters():
                    prox_term += (param - global_state[name]).pow(2).sum()

                loss = base_loss + (mu / 2.0) * prox_term
                loss.backward()
                optimizer.step()
                
                running_loss += base_loss.item() * images.size(0)
            
            avg_loss = running_loss / len(train_loader.dataset)
            epoch_losses.append(avg_loss)
        
        # Aggregate client models on the server (weighted by client dataset size)
        avg_state_dict = federated_averaging(models, client_weights)
        ...
        if (epoch + 1) % 5 == 0:
            print(f"FedProx Round {epoch+1}/{num_epochs} - Avg Loss: {np.mean(epoch_losses):.4f}")
...
print("\nTraining federated models with FedProx...")
...
federated_losses = train_fedprox(federated_models, federated_loaders)
...
plt.title('Training Loss vs Epoch - All Scenarios Comparison (FedProx)', fontsize=14, fontweight='bold')
...
def evaluate_model_on_subset(model, subset_images, subset_angles):
...
for model_name, model in models_dict.items():
    ...
    for subset_name, (subset_imgs, subset_angs) in subsets.items():
        mse = evaluate_model_on_subset(model, subset_imgs, subset_angs)
...
plt.title('Cross-Subset Generalization Performance\n(Error bars show standard deviation across subsets - FedProx)', 
          fontsize=14, fontweight='bold')
